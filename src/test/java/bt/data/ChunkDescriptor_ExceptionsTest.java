...
public class ChunkDescriptor_ExceptionsTest {
    ...
    public void testExceptions_NoFiles() {
        assertExceptionWithMessage(
                it -> new ChunkDescriptor(new DataAccess[0], 0, 0, new byte[20], 0),
    ...
    public void testExceptions_BlockSizeTooBig() {
        assertExceptionWithMessage(
                it -> new ChunkDescriptor(new DataAccess[1], 0, 5, new byte[20], 10),
    ...
    public void testExceptions_TooManyBlocks() {
        assertExceptionWithMessage(
                it -> new ChunkDescriptor(new DataAccess[1], 0, Long.MAX_VALUE, new byte[20], 1),
                "Integer overflow while constructing chunk: too many blocks");
    }

    @Test
    public void testExceptions_WriteEmptyBlock_NoException() {
        IChunkDescriptor chunk = new ChunkDescriptor(new DataAccess[]{mockDataAccess(10)}, 0, 10, new byte[20], 5);
        chunk.writeBlock(new byte[0], 0);
    }

    @Test
    public void testExceptions_Write_NegativeOffset() {
        IChunkDescriptor chunk = new ChunkDescriptor(new DataAccess[]{mockDataAccess(1)}, 0, 1, new byte[20], 1);
        assertExceptionWithMessage(
